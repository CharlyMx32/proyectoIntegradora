<template>
    <v-container class="pa-4">
      ...
      <v-dialog v-model="paymentSuccessDialog" max-width="400">
        <v-card>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="primary" @click="redirectToPedir">
              Cerrar
            </v-btn>
          </v-card-actions>
        </v-card>
  ...
  const selectedItem = ref(null) // Este se debe definir con los datos correctos
  ...
  const cancelPayment = () => {
    window.location.href = '/Pedir'  // Redirección directa a la página '/Pedir'
  }
...
// En tu función handlePayment, asegúrate de que la condición se cumpla correctamente
...
// Observa el diálogo y redirige correctamente después de 3.5 segundos
watch(paymentSuccessDialog, (newValue) => {
  if (newValue) {
    setTimeout(() => {
      redirectToPedir()
    }, 3500) // Espera 3.5 segundos antes de redirigir
  }
});

// Verifica la función de redirección para asegurarte de que está funcionando
const redirectToPedir = () => {
  console.log('Redirigiendo a /Pedir');
  router.push({ path: '/Pedir' }); // Usar router.push en lugar de window.location.href
}
  </script>
